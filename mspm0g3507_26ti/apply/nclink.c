												...
												float user9)
{
	...
	_cnt++;
	bt_app_output_buf[_cnt]=36;//数据长度
	_cnt++;
	...
	_cnt+=4;
	Float2Byte(&user9,bt_app_output_buf,_cnt);//33
	_cnt+=4;	
...
#define BLUETOOTH_TELEMETRY_V2_VERSION (2U)
#define BLUETOOTH_TELEMETRY_V2_PAYLOAD_LEN (38U)

static void bluetooth_put_u16(uint8_t *buffer, uint8_t *index, uint16_t value)
{
    buffer[(*index)++] = (uint8_t)(value & 0xFFU);
    buffer[(*index)++] = (uint8_t)(value >> 8);
}

static void bluetooth_put_i16(uint8_t *buffer, uint8_t *index, int16_t value)
{
    bluetooth_put_u16(buffer, index, (uint16_t)value);
}
...
void bluetooth_app_send_v2(const BluetoothTelemetryV2 *telemetry)
{
    ...
    bt_app_output_buf[index++] = PACK_HEAD;
    bt_app_output_buf[index++] = BLUETOOTH_TELEMETRY_V2_PAYLOAD_LEN;
    bt_app_output_buf[index++] = BLUETOOTH_TELEMETRY_V2_VERSION;
    bt_app_output_buf[index++] = telemetry->task_id;
    bt_app_output_buf[index++] = telemetry->phase;
    bt_app_output_buf[index++] = telemetry->route_mode;
    bt_app_output_buf[index++] = telemetry->flags;
    bt_app_output_buf[index++] = 0U;
    bluetooth_put_u16(bt_app_output_buf, &index, telemetry->sequence);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->ball_position_mm);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->ball_velocity_mm_s);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->target_position_mm);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->vehicle_measured_speed_mm_s);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->vehicle_command_speed_mm_s);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->vehicle_measured_acceleration_mm_s2);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->vehicle_command_acceleration_mm_s2);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->left_motor_speed_mm_s);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->right_motor_speed_mm_s);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->yaw_deg_centi);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->yaw_rate_dps_centi);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->servo_angle_deg_centi);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->servo_speed);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->feedforward_servo_deg_centi);
    bluetooth_put_i16(bt_app_output_buf, &index, telemetry->turn_compensation_servo_deg_centi);

    for (i = 1U; i < index; i++) sum = (uint8_t)(sum + bt_app_output_buf[i]);
    bt_app_output_buf[index++] = sum;
    bt_app_output_buf[index++] = PACK_TAIL;

    DL_DMA_setSrcAddr(DMA, DMA_CH0_CHAN_ID, (uint32_t)&bt_app_output_buf[0]);
    DL_DMA_setDestAddr(DMA, DMA_CH0_CHAN_ID, (uint32_t)(&UART_2_INST->TXDATA));
    DL_DMA_setTransferSize(DMA, DMA_CH0_CHAN_ID, index);
    DL_DMA_enableChannel(DMA, DMA_CH0_CHAN_ID);
}
