...
#define speed_breakaway_command_min_cmps  0.5f
#define speed_breakaway_detect_cmps       1.0f
#define speed_post_breakaway_integral     300.0f
...
static uint8_t speed_breakaway_seen[2]={0,0};
static int8_t speed_breakaway_direction[2]={0,0};

static void speed_control_release_breakaway_integral(uint8_t wheel)
{
	int8_t command_direction=0;

	if(speed_expect[wheel]>speed_breakaway_command_min_cmps)
		command_direction=1;
	else if(speed_expect[wheel]<-speed_breakaway_command_min_cmps)
		command_direction=-1;

	if(command_direction==0)
	{
		speed_breakaway_seen[wheel]=0;
		speed_breakaway_direction[wheel]=0;
		return;
	}

	if(speed_breakaway_direction[wheel]!=command_direction)
	{
		speed_breakaway_seen[wheel]=0;
		speed_breakaway_direction[wheel]=command_direction;
	}

	/* Each wheel accumulates enough integral to cross its own static-friction
	 * threshold. Once that wheel starts rolling, discard only its excess
	 * breakaway integral so the other wheel can continue building torque. */
	if(!speed_breakaway_seen[wheel]&&
	   speed_feedback[wheel]*(float)command_direction>=
	      speed_breakaway_detect_cmps)
	{
		if(speed_integral[wheel]*(float)command_direction>
		   speed_post_breakaway_integral)
		{
			speed_integral[wheel]=
				(float)command_direction*speed_post_breakaway_integral;
		}
		speed_breakaway_seen[wheel]=1;
	}
}

void speed_control_reset(void)
{
	speed_setup=0;
	speed_expect[0]=0;
	speed_expect[1]=0;
	...
	speed_output[0]=0;
	speed_output[1]=0;
	speed_output_delta[0]=0;
	speed_output_delta[1]=0;
	speed_output_last[0]=0;
	speed_output_last[1]=0;
	speed_error_last[0]=0;
	speed_error_last[1]=0;
	speed_breakaway_seen[0]=0;
	speed_breakaway_seen[1]=0;
	speed_breakaway_direction[0]=0;
	speed_breakaway_direction[1]=0;
}
	...
	switch(_speed_ctrl_mode)
	{
		case 1:
		{
			...
			{	
					...
					speed_control_release_breakaway_integral(0);
					...
					speed_control_release_breakaway_integral(1);
