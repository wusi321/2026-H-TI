...
/******************************************************************
 * 函 数 名 称：IIC_Start
 * 函 数 说 明：IIC起始时序（修复高速单片机下先高后低跳变）
 ******************************************************************/
void IIC_Start(void)
{
    SDA_OUT();

    // 确保总线释放，SCL和SDA初始全部为高电平
    SDA(1);
    SCL(1);
    delay_us(5);

    // 在 SCL 为高期间，将 SDA 拉低，形成完美的下降沿起始信号
    SDA(0);
    delay_us(5);
...
void IIC_Stop(void)
{
    ...
    // 在 SCL 为高期间，将 SDA 释放拉高，形成完美的上升沿停止信号
    SDA(1);
    delay_us(5);
}
...
/******************************************************************
 * 函 数 名 称：I2C_WaitAck
 * 函 数 说 明：等待从机应答（修正无符号数递减逻辑漏洞）
 ******************************************************************/
unsigned char I2C_WaitAck(void)
{
    unsigned char ack_flag = 20;
    SCL(0);
    SDA(1);
    SDA_IN();
...
uint8_t MPU6050ReadID(void)
{
    ...
    // 打印详细的读取返回值
    char result = MPU6050_ReadData(0x68, 0X75, 1, Re);
    printf("DEBUG: I2C Read Result = %d, Read Value = 0x%02X\r\n", result, Re[0]);
...
{
    // 1. 强制将 SCL 和 SDA 恢复为高电平闲置状态
    DL_GPIO_setPins(PORTA_PORT, PORTA_SCL_PIN | PORTA_SDA_PIN);

    // 2. 重新初始化 SDA 为带内部上拉的输出模式，确保起始信号可以拉高
    DL_GPIO_initDigitalOutputFeatures(PORTA_SDA_IOMUX,
                                      DL_GPIO_INVERSION_DISABLE, DL_GPIO_RESISTOR_PULL_UP,
                                      DL_GPIO_DRIVE_STRENGTH_LOW, DL_GPIO_HIZ_DISABLE);

    // 3. 重新初始化 SCL 为标准输出
    DL_GPIO_initDigitalOutput(PORTA_SCL_IOMUX);
